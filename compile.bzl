...
def _common_compile_module_args(
    ...
    # In worker mode, unit plugin interface files must be materialized before compilation;
    # they are not included in oneshot_args_for_file (which is skipped in worker mode),
    # so we include their hidden inputs here.
    args_for_file = cmd_args(
        [],
        hidden = non_haskell_sources + ([arg.plugin_params.unit.hidden] if is_worker_execute else []),
    )
