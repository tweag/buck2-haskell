...
def main():
    ...
    parser.add_argument(
        "--ghc-per-module-plugins-args-file",
        required=False,
        type=Path,
        help="""JSON file containing per-module plugins and their options.

           See `PluginFlags.mod_flags` in ghc_plugin.bzl for the expected format.
        """)
    parser.add_argument(
...
def obtain_target_metadata(args):
    ...
    if args.build_plan == None:
        ...
        output = run_ghc_depends(
            ...
            ghc_per_module_plugins_args_file=args.ghc_per_module_plugins_args_file,
        )
...
def run_ghc_depends(
    ...
    ghc_per_module_plugins_args_file: Path,
):
    with tempfile.TemporaryDirectory() as dname:
        ...
        if ghc_per_module_plugins_args_file:
            worker_args += ["--ghc-per-module-plugins-args-file={}".format(ghc_per_module_plugins_args_file)]
